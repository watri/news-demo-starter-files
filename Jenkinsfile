    ...
    stages {
        ...
        stage('deploy') {
            steps{
                ...
                chmod +x ./kubectl
                ./kubectl apply -f K8s/ -n wahab
                #./kubectl apply -f K8s/website-deployment.yaml -n wahab
                cat K8s/website-deployment.yaml | sed s/1.0.0/${BUILD_NUMBER}/g | ./kubectl apply -n wahab -f -
                #./kubectl apply -f K8s/website-service.yaml -n wahab
                '''
        }
    }
}
...
//                     sh 'kubectl delete -f /var/lib/jenkins/workspace/news-demo-starter-files_dev/K8s/website-deployment.yaml -n dev' 
//                 }
//             } 
//         } 
//         stage('Deploy to GKE') {
//             steps{
//                 catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
//                     sh 'kubectl apply -f /var/lib/jenkins/workspace/news-demo-starter-files_dev/K8s/ -n dev' 
//                 }            
//             }
//         }
